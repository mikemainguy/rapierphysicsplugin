...
function emitCollisionContinued(
  ...
  const eventType = PhysicsEventType.COLLISION_CONTINUED;
...
export function processCollisionEvents(
  ...
  // Track which pairs fired a Rapier event this frame (so we skip them for CONTINUED)
  const eventedPairs = new Set<string>();

  eventQueue.drainCollisionEvents((handle1: number, handle2: number, started: boolean) => {
    const pairKey = makePairKey(handle1, handle2);
    ...
    if (isSensor) {
      ...
      state.activeCollisionPairs.delete(pairKey);
  ...
  // Emit COLLISION_CONTINUED for active pairs that had no Rapier event this frame
  for (const pairKey of state.activeCollisionPairs) {
    if (eventedPairs.has(pairKey)) continue;
    const [h1Str, h2Str] = pairKey.split('_');
    emitCollisionContinued(state, Number(h1Str), Number(h2Str));
  }
}
